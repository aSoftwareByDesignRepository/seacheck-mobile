...
class LocationModule : Module(), LifecycleEventListener, SensorEventListener, ActivityEventListener {
  ...
  private val mLocationRequests = HashMap<Int, LocationParams>()
  ...
  override fun definition() = ModuleDefinition {
    ...
    AsyncFunction("watchPositionImplAsync") { watchId: Int, options: LocationOptions, promise: Promise ->
      ...
      val locationParams = LocationHelpers.prepareLocationParams(options)
      val showUserSettingsDialog = options.mayShowUserSettingsDialog

      LocationHelpers.requestContinuousUpdates(this@LocationModule, locationParams, watchId, promise)
    }
    ...
    AsyncFunction("enableNetworkProviderAsync") Coroutine { ->
      ...
      throw LocationSettingsUnsatisfiedException()
    }
  ...
  private fun getCurrentPositionAsync(options: LocationOptions, promise: Promise) {
    ...
    val locationParams = LocationHelpers.prepareLocationParams(options)
    LocationHelpers.requestSingleLocation(mContext, locationParams, promise)
  }

  fun requestLocationUpdates(locationParams: LocationParams, requestId: Int?, callbacks: LocationRequestCallbacks) {
    val provider = LocationHelpers.chooseProvider(mLocationManager, locationParams) ?: run {
    ...
    if (requestId != null) {
      mLocationListeners[requestId] = listener
      mLocationRequests[requestId] = locationParams
    }

    try {
      mLocationManager.requestLocationUpdates(provider, locationParams.interval, locationParams.distance, listener, Looper.getMainLooper())
  ...
  private fun addPendingLocationRequest(locationParams: LocationParams, listener: LocationActivityResultListener) {
    ...
    if (mPendingLocationRequests.size == 1) {
      resolveUserSettingsForRequest(locationParams)
    }
  }
   ...
   */
  private fun resolveUserSettingsForRequest(locationParams: LocationParams) {
  ...
  private fun startHeadingUpdate() {
    ...
    if (lastLocation != null) {
      ...
      val params = LocationParams(accuracy = LocationAccuracy.HIGH, distance = 0f, interval = 1000)
      val provider = LocationHelpers.chooseProvider(mLocationManager, params) ?: return
      val listener = object : LocationListener {
        override fun onLocationChanged(location: Location) {
      ...
      try {
        mLocationManager.requestLocationUpdates(provider, params.interval, params.distance, listener, Looper.getMainLooper())
  ...
  private fun pauseLocationUpdatesForRequest(requestId: Int) {
    val listener = mLocationListeners[requestId]
    if (listener != null) {
  ...
  private fun resumeLocationUpdates() {
    for (requestId in mLocationListeners.keys) {
      val listener = mLocationListeners[requestId] ?: return
      val params = mLocationRequests[requestId] ?: return
      val provider = LocationHelpers.chooseProvider(mLocationManager, params) ?: continue
      try {
        mLocationManager.requestLocationUpdates(provider, params.interval, params.distance, listener, Looper.myLooper())
      } catch (e: SecurityException) {
  ...
  private suspend fun getLastKnownLocation(): Location? {
    return suspendCoroutine { continuation ->
      try {
        val gps = mLocationManager.getLastKnownLocation(LocationManager.GPS_PROVIDER)
        val network = mLocationManager.getLastKnownLocation(LocationManager.NETWORK_PROVIDER)
  ...
  private suspend fun startMotionActivityWatch(watchId: Int) {
    throw MotionActivityUnavailableException(null)
  }
