#!/usr/bin/env python3
"""Finish SeaCheck R2 from current AVD state → en+de phone shots + visual-ready."""
...
def tap_id(rid=None, text=None, scrolls=2):
...
def dismiss_sheets():
    xml = dump()
    for rid in ("confirm.cancel", "confirm.sheet.close", "map.topAlert.dismiss", "feedback.banner"):
    ...
    for text in ("Not yet", "Noch nicht"):
        ...
        if hit:
            ...
            xml = dump()
    # blind close corners
    for x, y in ((1000, 1490), (980, 560), (1000, 180), (956, 178)):
        tap(x, y, 0.15)
...
def open_downloads() -> bool:
    dismiss_sheets()
    # Prefer Maestro when available
    flow = Path("/tmp/seacheck-open-dl.yaml")
    flow.write_text(
        f"""appId: {PKG}
---
- launchApp
...
"""
    ...
    r = subprocess.run(
        [MAESTRO, "--device", S, "test", str(flow)],
        text=True,
        capture_output=True,
        timeout=90,
    )
    print("MAESTRO_DL", r.returncode, (r.stdout or "")[-400:], flush=True)
    xml = dump()
    if "downloads.download" in xml or "Offline charts" in xml or "Offline-Karten" in xml:
        return True
    # Fallback blind: More then first overflow row
    tap(945, 2029, 1.2)
    for y in (1480, 1560, 1640, 1400, 1720):
        tap(540, y, 1.0)
        xml = dump()
        if "downloads.download" in xml or "Download pack" in xml or "Paket laden" in xml:
            print("BLIND_DL", y, flush=True)
            return True
        if "settings." in xml or "MAP BEHAVIOUR" in xml:
            tap(80, 140, 0.5)
            tap(945, 2029, 1.0)
    return "downloads" in dump().lower()


def seal() -> bool:
    if not open_downloads():
        print("FAIL open downloads", flush=True)
        return False
    time.sleep(1.0)
    xml = dump()
...
def run_locale(loc: str):
    ...
    # deactivate before downloads to avoid confirm.sheet
    sh("shell", "am", "force-stop", PKG, t=8)
    sh(
        "shell",
        f"sqlite3 {DB} \"UPDATE passages SET is_active=0;\"",
        t=10,
    )
    focus()
    time.sleep(2)
...
def write_ready():
    ...
    for dbg in DOCS.glob("_dbg-*.png"):
        dbg.unlink()
    for dbg in DOCS.glob("_view-*.png"):
        dbg.unlink()
    c = {}
    ...
    ready = {
        ...
        "capture_script": "mobile/seacheck/scripts/capture-play-r2-finish-land.py",
        "r2_fixes": [
            "underway SOG via emu geo velocity; dismiss Jump filtered",
            "open-water chart hero",
            "named Kieler Foerde Laboe with ≥3 legs (SQLite seed)",
            "deactivate before downloads; dismiss confirm.sheet",
            "seal kiel-bay before #4; safety bottom-anchor",
        ],
