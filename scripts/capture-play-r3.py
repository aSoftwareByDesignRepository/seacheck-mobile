#!/usr/bin/env python3
"""SeaCheck Play R3 — fix R2 REJECT: distinct EN+DE shots, no lock/onboarding clones, Safety #6."""
...
FORBIDDEN_UI = (
    ...
    "Hold to unlock",
    "Gedrückt halten",
    # Do NOT match map chrome "Battery 100%" — only onboarding battery step
    "Akku & Hintergrund",
    "Battery & background",
    "Step 3 of 4",
    "Schritt 3 von 4",
...
def hold(x, y, ms=1800):
    """Press-and-hold (unlock overlay needs ~1500ms)."""
...
def fit(im: Image.Image) -> Image.Image:
    """Cover-crop 1080x1920 bottom-anchored (keeps Safety CTA)."""
    im = im.convert("RGB")
    sw, sh_ = im.size
...
def fit_top(im: Image.Image) -> Image.Image:
    """Cover-crop top-anchored — keeps map tip banners fully in frame when present."""
    im = im.convert("RGB")
    sw, sh_ = im.size
    scale = max(TW / sw, TH / sh_)
    nw, nh = int(sw * scale), int(sh_ * scale)
...
def dismiss_tips():
    """Dismiss download tip / jump / feedback — never leave mid-cropped banner."""
    for _ in range(4):
        xml = dump()
        hit = (
        ...
        if not hit:
            # × on tip often top-right of banner
...
def patch_settings_dismiss_tips():
    """Persist downloadHintDismissed so map tips do not reappear mid-crop.

    Always push via SQL file — adb shell strips JSON quotes and corrupts settings.
    """
    r = sh(
        "shell",
    ...
    print("SETTINGS tips dismissed", flush=True)
...
"""
    ...
    if "screen.downloads" in xml or ("Offline" in xml and "downloads." in xml):
        return True
    # adb fallback — More sheet needs settle time; dump often empty mid-animation
    tap(945, 2029, 1.6)
    for _ in range(8):
        xml = dump()
        hit = (
...
def seal() -> bool:
    """Start kiel-bay if needed; wait until packId present AND download chrome gone."""
    ...
    if not already:
        ...
        for _ in range(20):
            xml = dump()
            hit = (
                find(xml, rid="downloads.download.kiel-bay")
                or find(xml, text="Download pack")
                or find(xml, text="Paket laden")
                or find(xml, text="Kieler Bucht")
            )
            if find(xml, rid="downloads.download.kiel-bay") or find(xml, text="Download pack") or find(
                xml, text="Paket laden"
            ):
                hit = (
                    find(xml, rid="downloads.download.kiel-bay")
                    or find(xml, text="Download pack")
                ...
                started = True
                break
            if find(xml, text="Kieler Bucht"):
                kb = find(xml, text="Kieler Bucht")
                tap(kb[0], min(kb[1] + 180, 1900), 0.5)
            swipe(540, 1600, 540, 700, 300)
    ...
    while time.time() < end:
        xml = dump()
        # App asks to stay on Map while saving
        if download_chrome_visible(xml):
            if "screen.map" not in xml:
                tap(135, 2029, 0.5)
            print("SEAL wait chrome…", flush=True)
            time.sleep(5)
            continue
        if offline_has_kiel():
            if open_downloads():
...
def onboard_capture_safety(raw: Path):
    """Capture #6 on disclaimer step ONLY, then finish onboarding (never Battery/Fertig as #6)."""
    # Wait until disclaimer is actually visible (boot can skip past if we grab too early)
    disclaimer_ok = False
    for i in range(40):
        time.sleep(0.6)
        xml = dump()
        if (
            ...
            break
        # If already on map, we missed disclaimer — force reinstall path fails; abort locale
    ...
    # Scroll so title + body are in view; CTA stays sticky at bottom
    for _ in range(2):
    ...
    if not (tap_id(rid="onboarding.disclaimer.continue", scrolls=1) or tap_id(text="I understand", scrolls=1) or tap_id(
        text="Verstanden — weiter", scrolls=1
    )):
        tap(540, 1962, 1.0)
    time.sleep(0.8)
    # location
...
def run_locale(loc: str):
    ...
    sh("shell", "cmd", "locale", "set-app-locales", PKG, "--locales", lang, t=8)
    # CRITICAL: do NOT grant location yet — resumeStep skips disclaimer when FINE is granted
    sh("shell", "pm", "grant", PKG, "android.permission.POST_NOTIFICATIONS", t=5)
    focus()
    onboard_capture_safety(raw)
    # Now grant location for map/SOG
    grant()
    focus()
    time.sleep(1.0)

    # #1 map hero — tips dismissed, underway SOG, NEVER lock
    tap(135, 2029, 0.5)
    ensure_unlocked()
    dismiss_tips()
    geo(16)
    ...
    ensure_unlocked()
    dismiss_tips()
    # #2 active passage on chart
    tap(135, 2029, 0.6)
    geo(10)
    dismiss_tips()
    # open passage preview on map if available
    ...
    if "Laboe" not in xml and NAME.split()[0] not in xml and "passage.detail" not in xml:
        print("WARN passage detail weak UI", flush=True)
    cap(raw / "03-passage-detail.png")

    # deactivate to avoid arrival sheet during downloads
    sh("shell", "am", "force-stop", PKG, t=8)
    ...
    # #5 offline map with underway geo
    ...
    # en #2/#3 must differ substantially
...
def write_locale_outputs(loc: str, raw: Path):
    ...
    mapping = {
        "01-map-hero.png": ("phone-01-map.png", "1.png", "top"),  # top-anchor avoids clipping tips if any remain
        "02-map-passage.png": ("phone-02-passage-map.png", "2.png", "top"),
        "03-passage-detail.png": ("phone-03-passage.png", "3.png", "bottom"),
...
def write_ready():
    ...
    ready = {
        ...
        "screenshots": {
            ...
            "sevenInch": 0,
            "tenInch": 0,
            "note": "R3: wipe DE Fertig clones; EN unlock+passage; Safety#6 not Battery; tips dismissed; sealed kiel-bay; lock held.",
        },
        ...
        "capture_script": "mobile/seacheck/scripts/capture-play-r3.py",
        "r3_fixes": [
            "DE 1–6 independent product frames (no Fertig/Akku clones)",
            "EN #2/#3 active passage map + detail — no Screen locked",
            "EN+DE #6 Safety/Sicherheitshinweis (not Battery onboarding)",
            "dismiss download tips before map/downloads grabs; top-anchor crop for tip safety",
            "underway SOG; sealed kiel-bay lead; named Kieler Foerde Laboe ≥3 legs",
        ],
        ...
        json.dumps(
            {
                ...
                "apk": ready["apk"],
                "updated_at": now,
                "notes": "R3 landed; lock held for critic — do not kill AVD",
            },
            indent=2,
        )
        + "\n"
    )
    print("WROTE visual-ready ready_for_critic:true round:3", flush=True)
